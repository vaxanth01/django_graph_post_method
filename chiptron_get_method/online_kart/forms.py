...
#     end_time = forms.TimeField(required=False, input_formats=['%H:%M'])
# class ItemCodeForm(forms.Form):
#     item_code = forms.ChoiceField(choices=[], required=False)
#     start_date = forms.DateField(required=False, widget=forms.DateInput,initial=datetime.now()(attrs={'type': 'date'}))
...
#         self.fields['start_date'].initial = current_datetime.date()
#         self.fields['start_time'].initial = current_datetime.time()
from django import forms
...
class ItemCodeForm(forms.Form):
    ...
    # start_time = forms.TimeField(required=False, initial=datetime.now().time(), widget=forms.TimeInput(attrs={'type': 'time'}))
